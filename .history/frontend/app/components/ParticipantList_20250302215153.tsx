  ...
  const fetchParticipants = async (newSortOrder: "asc" | "desc", page: number, nameQuery: string, icdQuery: string) => {
    try {
      const response = await fetch(
        `http://localhost:5000/participants?page=${page + 1}&limit=100&sort=icdCount&sortOrder=${newSortOrder}&sortOnlyOnPage=${page + 1}&search=${nameQuery}&icdSearch=${icdQuery}`
      );
...
const toggleSort = () => {
  setSortOrder((prevOrder) => {
    ...
    fetchParticipants(newOrder, currentPage, searchName, searchICD); // ✅ 立即触发请求
    return newOrder;
  });
};
