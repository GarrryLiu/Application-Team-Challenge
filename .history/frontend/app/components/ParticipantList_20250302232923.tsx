  ...
  const fetchParticipants = async (
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/participants?page=${page + 1}&limit=${rowsPerPage}&sort=${sortType}&sortOrder=${newSortOrder}&sortOnlyOnPage=${page + 1}&search=${nameQuery}&cdSearch=${encodeURIComponent(icdQuery)}`
      );
