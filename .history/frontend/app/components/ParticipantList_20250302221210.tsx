...
const ParticipantList = () => {
  ...
  const fetchParticipants = async (newSortOrder: "asc" | "desc", page: number, nameQuery: string, icdQuery: string) => {
    try {
      ...
      console.log("Fetched Data (Sorted by ICD Count):", data.data); // ✅ 打印返回数据，看看是否真的排序了
  
      setParticipants(data.data); // ✅ 直接更新 participants
      setTotalParticipants(data.total); // ✅ 确保总数正确
    } catch (error) {
...
const toggleSort = () => {
  setSortOrder((prevOrder) => {
    ...
    fetchParticipants(newOrder, currentPage, searchName, searchICD); // ✅ 立即获取排序后的数据
  ...
  return (
    <Box sx={{ maxWidth: 1500, margin: "auto", mt: 4 }}>
      <TableContainer component={Paper} .5)" }}>
        <Table>
          <TableHead>
            <TableRow>
