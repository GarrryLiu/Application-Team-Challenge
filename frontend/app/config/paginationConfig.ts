 ...
 */
export const calculateRowsPerPageOptions = (totalItems: number): RowsPerPageOption[] => {
  ...
  if (totalItems < 50) {
    const filteredOptions = baseOptions.filter(option => option <= Math.min(totalItems, 25));
...
export const DEFAULT_ROWS_PER_PAGE_OPTIONS: RowsPerPageOption[] = [5, 10, 25, 50, { label: 'All', value: -1 }];
