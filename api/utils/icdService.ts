...
const conditionCache: Record<string, string> = {};
...
const ICD_CATEGORIES = [
  {
    ...
    name: "infectious",
    displayName: "A00-B99 Infectious & Parasitic Diseases",
  },
  {
    range: { start: "C00", end: "D48" },
    name: "malignancy",
    displayName: "C00-D48 Neoplasms, Malignancy",
  },
  {
    range: { start: "D50", end: "D89" },
    name: "hematologic",
    displayName: "D50-D89 Hematologic & Immune Disorders",
  },
  {
    range: { start: "E00", end: "E90" },
    name: "endocrine",
    displayName: "E00-E90 Endocrine, Metabolic",
  },
  {
    range: { start: "F00", end: "F99" },
    name: "mental",
    displayName: "F00-F99 Mental & Behavioral Disorders",
  },
  {
    range: { start: "G00", end: "G99" },
    name: "neurological",
    displayName: "G00-G99 Neurological Disorders",
  },
  {
    range: { start: "H00", end: "H59" },
    name: "ophthalmological",
    displayName: "H00-H59 Ophthalmological Disorders",
  },
  {
    range: { start: "H60", end: "H95" },
    name: "otological",
    displayName: "H60-H95 Otological Disorders",
  },
  {
    range: { start: "I00", end: "I99" },
    name: "cardiovascular",
    displayName: "I00-I99 Cardiovascular Diseases",
  },
  {
    range: { start: "J00", end: "J99" },
    name: "respiratory",
    displayName: "J00-J99 Respiratory Diseases",
  },
  {
    range: { start: "K00", end: "K93" },
    name: "digestive",
    displayName: "K00-K93 Digestive Disorders",
  },
  {
    range: { start: "L00", end: "L99" },
    name: "dermatological",
    displayName: "L00-L99 Dermatological Diseases",
  },
  {
    range: { start: "M00", end: "M99" },
    name: "musculoskeletal",
    displayName: "M00-M99 Musculoskeletal Disorders",
  },
  {
    range: { start: "N00", end: "N99" },
    name: "urogenital",
    displayName: "N00-N99 Urogenital Diseases",
  },
  {
    range: { start: "O00", end: "O99" },
    name: "pregnancy",
    displayName: "O00-O99 Pregnancy & Perinatal Conditions",
  },
  {
    range: { start: "P00", end: "P96" },
    name: "perinatal",
    displayName: "P00-P96 Perinatal & Neonatal Conditions",
  },
  {
    range: { start: "Q00", end: "Q99" },
    name: "congenital",
    displayName: "Q00-Q99 Congenital Malformations",
  },
  {
    range: { start: "R00", end: "R99" },
    name: "symptoms",
    displayName: "R00-R99 Symptoms & Abnormal Clinical Findings",
  },
  {
    range: { start: "S00", end: "T98" },
    name: "injury",
    displayName: "S00-T98 Injury, Poisoning & External Causes",
  },
  {
    range: { start: "V00", end: "Y99" },
    name: "external",
    displayName: "V00-Y99 External Causes of Morbidity & Mortality",
  },
  {
    range: { start: "Z00", end: "Z99" },
    name: "factors",
    displayName:
      "Z00-Z99 Factors Influencing Health Status & Preventive Medicine",
  },
];
...
export async function getConditionName(icdCode: string): Promise<string> {
  ...
  try {
    ...
    const conditionName = await fetchFromSearchAPI(icdCode);
    if (conditionName) {
    ...
    const docConditionName = await fetchFromDocAPI(icdCode);
    if (docConditionName) {
 ...
 */
async function fetchFromSearchAPI(icdCode: string): Promise<string | null> {
  try {
 ...
 * @returns The condition name or null if not found
 */
async function fetchFromDocAPI(icdCode: string): Promise<string | null> {
  try {
    const response = await axios.get(API_ENDPOINTS.DOC, {
      params: { code: icdCode },
      timeout: 5000, // 5 second timeout
    });
 ...
 */
function isCodeInRange(code: string, start: string, end: string): boolean {
  // Extract the letter part and number part
  const codeLetters = code.match(/^[A-Z]+/)?.[0] || "";
  const codeNumbers = code.replace(/^[A-Z]+/, "");

  const startLetters = start.match(/^[A-Z]+/)?.[0] || "";
  const startNumbers = start.replace(/^[A-Z]+/, "");

  const endLetters = end.match(/^[A-Z]+/)?.[0] || "";
 ...
 */
export function getDiseaseCategory(icdCode: string): string {
  // Handle non-standard codes
  if (nonStandardICDCodes.has(icdCode)) {
    return "other";
  }
  ...
  const category = ICD_CATEGORIES.find((cat) =>
    isCodeInRange(normalizedCode, cat.range.start, cat.range.end),
  );
